...
$$ P[n_1  | N = \textbf{n}] =  {\frac{1}{n}}  , for\ \textbf{1} \leq n_1 \leq \textbf{n} $$
The likelihood of observing a given sequence of tanks is given by:

$$ P[n_1 \dots n_k | N = \textbf{n}] =  \big( {\frac{1}{n}}^k \big) , for\ \textbf{1} \leq n_1, \dots,n_k \leq \textbf{n} $$
...
For our tank problem, a good estimator turns out to be:

$$ \hat{N} = m \cdot \frac{k+1}{k} - 1  $$
where $  m = Max(n_1,\dots,n_k) $

See [here](https://www.youtube.com/watch?v=quV-MCB8Ozs) to learn how this estimator was derived.
...
Consider the tank numbers we observe are 324,167,129 and 418.Then the estimated number of tanks are
...
This estimator also has the nice property of being the  **Minimum Variance Unbiased Estimator**. In the below two sections, we take a quick detour to understand what these term means. Feel free to skip over these sections.
...
It may be easier to illustrate this idea by considering Variance. Now, Variance is a measure of dispersion that almost everyone is familiar with. It is given by :

$$ Var(X) = \frac{1}{n} \sum_{i=1}^{n}(x_i - \mu)^2   $$
Despite the above definition of variance, the unbiased estimate of variance is given by. See [link](https://en.wikipedia.org/wiki/Variance#Population_variance_and_sample_variance) for why.  
...
We can determine the upper bond of the confidence interval by setting above value equal to $ 1 - \alpha $ and solving for $ N $ which gives:
...
cat("Th estimated number of total tanks based on this number is",N_hat[1])
```
...
Assuming a \alpha (=5%) significance level, this rejection region is given by the shaded area in the figure below. This can be determined by calculating the value of $m$ where the cumulative mass function has a value of \alpha.
...
As noted in earlier section, the maximum tank number observed by the first spy is 418. So, we do not reject the null hypothesis that the actual number of tanks is 500. Now, what are the possible values of N, where we would reject the null hypothesis given we observed a maximum of 418 ?
...
Given the sampling distribution for a hypothesized value of $ N $ in Figure 1, there is still an $ \alpha = 5\% $ chance that we will observe a value of $m $ in the rejection region and incorrectly reject the null hypothesis.
...
In other words, we want the 95% confidence interval to capture a range more like the lower image  rather than the upper image.
...
Assuming a  $ \alpha (=5%) $ significance level, this new rejection region is given by the shaded area in the figure below. This can be determined by calculating the value of $ m $ where the cumulative probability mass function has a value of $ 1 - \alpha $ 


$$ (\frac{m'}{N})^k =  1 -\alpha $$
$$ m' = (1 -\alpha)^{\frac{1}{k}}N  $$
Let our null hypothesis be that the number of tanks is 500.
...
So if the maximum serial number we observe $m$ greater than 493, we can reject the null hypothesis $ H0: N = 500 $
...
Not withstanding the fact that the definition of confidence intervals is confusing, we see that the two ways of computing the confidence interval can give answers of wildly varying utility. There is also no technical reason why one confidence interval is preferable compared to the other.

## The Bayesian Approach

Using a Bayesian approach we want to calculate the posterior distribution of the number of tanks the enemy has, given the specific sequence of tanks a spy observed.

Although this specific problem can be solved analytically as shown [here](https://www.mjandrews.org/blog/germantank/), a more general approach is to solve this using a Markov Chain Monte Carlo sampler. This can be done by using the software package `JAGS` and the R interface to JAGS `rjags`.
...
plotPost( codaSamples1[,"N"] , main="N" , xlab="N" )
```
In this case, we see that the posterior distribution is much wider given the tanks observed by the spy.
...
Given the intelligence information we have a the end of the war, we see that we can arrive at a much narrower posterior estimate.



## Rough Work

In this case the lower bound of the desired confidence interval can be calculating by starting at $ n = m $ and counting up to $n = m + i $ such the cumulative probability of observing a maximum value $ = m $ across these values is $ \alpha $.

Note that the the probability of observing a maximum of $ Max = m $ out of a sample of $ k $ numbers from 1 to $ n  $ is given by

$ P[Max = m]  = P[Max \leq m] - P[Max \leq m-1] = (\frac{m}{n})^k - (\frac{m-1}{n})^k $



The lower bound of the confidence interval is then given by  $ m+i+ 1 $. This can be computed using the function below.

```{r}
#Function to calculate the 
bound <- function(max,alpha,k){
  cum_prob <- 0
  n <- max
  while(cum_prob < alpha){
    prob <-  (max/n)^k - ((max-1)/n)^k #P[max = m]
...
```{r}
lower_bound(313,0.05,4)
```


# Reparameterizing a neg binomial distribution
...
```{r}
gammaShRaFromMeanSD(500,100)
```

```{r}
# hist(rgamma(1000,16,0.01))
hist(rgamma(1000,25,0.05))
```
